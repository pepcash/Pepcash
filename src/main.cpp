...
#if defined(NDEBUG)
# error "Pepcash cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x6a7d0733f8e1e940f26aa1d95d390ef8281cb3c06c2e96b2b7a15ae673ea395f");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Pepcash: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Pepcash Signed Message:\n";
...
{
    // Pepcash: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 210000); // Pepcash: 210k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Pepcash: 1 days
static const int64 nTargetSpacing = 5 * 60; // Pepcash: 5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Pepcash: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Pepcash: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x2524a071174cbd19c8ee2534d04c324a4229986d80b426f3d01d75324852354f");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "NY Times 15/April/2018 Pentagon Declares Syria Strike Successful";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("041a304a56a95be478ab1af58ff2960f31d0fb492a687d3d406e97679b193f58145ad65f590c517e206c16fa59a6ccdfaa51a305343f9126c887added5cd4d9731") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1523859117;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085299958;

        if (fTestNet)
        {
            block.nTime    = 1523794654;
            block.nNonce   = 1524089078;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x71be2481accf5e1a155180ed6aa3c9a2a9799343e608e3c8d8a09ea8c9cc1ea3"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfe, 0xc2, 0xb8, 0xdd }; // Pepcash: increase each by adding 2 to Litecoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("PepcashMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("PepcashMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static PepcashMiner(CWallet *pwallet)
{
    printf("PepcashMiner started\n");
    ...
    try { loop {
        ...
        printf("Running PepcashMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("PepcashMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&PepcashMiner, pwallet));
}
